...
const adjustBrightness = (color, el, options) => {
  ...
  if (options.isBgColor) {
    ...
    if (!el.getAttribute(BGIMAGEATTR)) {
      ...
      if (window.demo2) {
        if ((!isParentElementBgColorLight && perceivedBrightness < textLimitLowBright) || (isParentElementBgColorLight && perceivedBrightness < whiteColorBrightness && perceivedBrightness >= limitBright)) {
          // hsl[2] = 90 - hsl[2];
          let _rgb = rgb;
          if (hsl[2] < 40) {
            hsl[2] = 90 - hsl[2];
          ...
          if (hsl[1] === 0) {
            ...
            const relativeBrightnessRatio = (textLimitLowBright * 1000) / (_rgb[0] * 299 + _rgb[1] * 587 + _rgb[2] * 114);

            let newTextR = Math.min(255, _rgb[0] * relativeBrightnessRatio);
            let newTextG = Math.min(255, _rgb[1] * relativeBrightnessRatio);
            let newTextB = Math.min(255, _rgb[2] * relativeBrightnessRatio);

            if (newTextG == 0) {
              newTextG = (textLimitLowBright * 1000 - newTextR * 299 - newTextB * 114) / 587;
            } else if (newTextR == 0) {
              newTextR = (textLimitLowBright * 1000 - newTextG * 587 - newTextB * 114) / 299;
            } else if (newTextB == 0) {
              newTextB = (textLimitLowBright * 1000 - newTextR * 299 - newTextG * 587) / 114;
...
const convert = el => {
  ...
  try {
    ...
    ((styles.cssText && styles.cssText.split(";")) || [])
      .map(cssStr => {
      ...
      .forEach(([key, value]) => {
        ...
        if ((/^background/.test(key) || /^(-webkit-)?border-image/.test(key)) && /url\([^\)]*\)/i.test(value)) {
          ...
          value = value.replace(/^(.*?)url\(([^\)]*)\)(.*)$/i, (matches, match1, match2, match3) => {
            if (el.getAttribute(BGIMAGEATTR) !== "1") {
              // 避免重复setAttribute
...
const adjustBrightness2 = options => {
  if (options.isColor) {
    const rgb = color.rgb().array();
    const hsl = color.hsl().array();
    const alpha = color.alpha();
    ...
    .forEach(([key, value]) => {
      ...
      if (colorReg.test(value)) {
        value = value.replace(colorReg, match => {
          ...
          const isBgColor = /^background/.test(key);
          const isTextColor = key === "color";
          let retColor = adjustBrightness2({
            el,
            isColor: true,
            isBgColor,
            isTextColor,
      ...
      const imageReg = /^(.*?)url\(([^\)]*)\)(.*)$/i;
      if ((/^background/.test(key) || /^(-webkit-)?border-image/.test(key)) && imageReg.test(value)) {
        value = value.replace(/^(.*?)url\(([^\)]*)\)(.*)$/i, (matches, match1, match2, match3) => {
          if (el.getAttribute(BGIMAGEATTR) !== "1") {
            // 避免重复setAttribute
            getChildrenAndIt(el).forEach(dom => dom.setAttribute(BGIMAGEATTR, "1"));
...
var article_dark1 = document.getElementById("article_dark1");
var article_dark2 = document.getElementById("article_dark2");
article_dark1.innerHTML = article_light.innerHTML;
article_dark2.innerHTML = article_light.innerHTML;

var cssAll = [];
article_dark1.querySelectorAll("*").forEach(el => {
  cssAll.push(convert(el));
});
article_dark2.querySelectorAll("*").forEach(el => {
  cssAll.push(convert2(el));
});
writeStyle(cssAll.join(""));
